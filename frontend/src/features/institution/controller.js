...
// export default 
module.exports = ($injector, $scope, $state) => {
    controller($injector, $scope, $state);
    require('./maintainer/controller');
    require('./maintained/controller');
    require('./course/controller');
    require('./procurator/controller');
    require('./auxiliary/controller');
};

function controller($injector, $scope, $state) {
    ...
    (async function () {
        ...
        $scope.$watch(function () {
            ...
            $scope.stateName = newVal;
            let btnList = document.querySelectorAll("#ips-list-watch button");
            btnList.forEach((btn) => {
                const state = btn.getAttribute('href').split('/').pop();
                // const state = btn.outerHTML.match(/.*\/(.*)"/)[1];
                if (state == newVal)
        ...
        window.onload = () => {
            let buttonList = document.querySelectorAll("#ips-list-watch button");
            buttonList.forEach((btn) => {
                if (btn.outerHTML.match(/.*\/(.*)"/)[1] == $state.current.name) {
                    btn.classList.add('active');
                } else {
                    btn.classList.remove('active')
                }
        ...
        // function fnSetEventListener(selector) {
        //     let buttonList = document.querySelectorAll(selector);
        //     buttonList.forEach((btn, idx, lst) => {
        //         if (btn.outerHTML.match(/.*\/(.*)"/)[1] == $state.current.name) {
        //             fnClassActiveRemove(lst);
        //             btn.classList.add('active');
        //         }
        ...
        // function fnClassActiveRemove(buttonList) {
        //     buttonList.forEach((btn) => {
        //         btn.classList.contains('active') ? btn.classList.remove('active') : undefined;
        //     })
        // }
